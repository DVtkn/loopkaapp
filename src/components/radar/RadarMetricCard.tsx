...
export const RadarMetricCard: React.FC<RadarMetricCardProps> = ({
  ...
  const getStatusBadge = (score: number) => {
    if (score >= 75) {
      return {
        label: 'Синхронный резонанс',
        className: 'bg-emerald-500/15 text-emerald-600 dark:text-emerald-400 border-emerald-500/20',
        detail: 'Высокое совпадение ценностей и схожие реакции, создающие ощущение глубокого взаимопонимания и надёжности.',
      };
    }
    if (score >= 50) {
      return {
        ...
        detail: 'Взаимодополняющие различия и гибкость, которые обогащают союз при открытом диалоге.',
      };
    }
    return {
      label: 'Точка рассинхрона',
      className: 'bg-rose-500/15 text-rose-600 dark:text-rose-400 border-rose-500/20',
      detail: 'Разные автоматические реакции или ожидания, требующие бережного проговаривания по правилу Готтмана.',
    };
  };
